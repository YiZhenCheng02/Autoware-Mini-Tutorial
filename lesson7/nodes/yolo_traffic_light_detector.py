...
class YoloTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        # and collect the ids of the stop lines that intersect with it into stop_line_ids_on_path.
        if len(local_path_msg.waypoints) > 1:
            path_coords = [(wp.position.x, wp.position.y) for wp in local_path_msg.waypoints]
            path_linestring = shapely.LineString(path_coords)
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if stop_line_ids_on_path:
            ...
            if map_rois:
                # Run the YOLO model on the image to get yolo_rois, classes and scores then match them with map_rois:
        ...
        for stop_line_id in stop_line_ids_on_path:
            for traffic_light_id, traffic_light_coords in self.traffic_lights[stop_line_id].items():
                ...
                for x, y, z in traffic_light_coords:
                    ...
                    # Break if point is behind the camera or outside the image bounds
    ...
    def match_map_and_yolo_rois(self, map_rois, yolo_rois, yolo_classes, yolo_scores):
        ...
        for stop_line_id, traffic_light_id, x1_map, x2_map, y1_map, y2_map in map_rois:
            ...
            # Find the YOLO detection that best matches this map ROI.
            ...
            for idx, (cls, score, yolo_roi) in enumerate(zip(yolo_classes, yolo_scores, yolo_rois)):
                # calculate the IOU between the map ROI and the YOLO box
                iou_score = self.calculate_iou(np.array([[x1_map, y1_map, x2_map, y2_map]]), yolo_roi[np.newaxis, :])[0][0]
                
                # skip matches with IOU below self.iou_threshold
            ...
            if matched_roi is None:
                ...
                cls = matched_roi[0]
                tfl_result.status = CLASS_TO_TLRESULT[cls]
                tfl_result.status_text = CLASS_TO_STRING[cls]
